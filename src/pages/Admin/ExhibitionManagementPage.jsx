...
const ExhibitionManagementPage = () => {
  ...
  const [activeTab, setActiveTab] = useState(
    location.state?.activeTab || "schedule"
  );
  ...
  // 🎯 التعديل هنا: جميع التبويبات الأساسية للمشروع
  const allTabs = [
  ...
  // 🎯 تصفية التبويبات: إذا كان المستخدم سكرتيرة، نقوم بحذف تبويب طلبات البطاقات فوراً
  const tabs = isSecretary 
    ? allTabs.filter(tab => tab.id !== "requests") 
    : allTabs;
  ...
  return (
    <div>
      ...
      <div className="container p-6 mt-20">
        ...
        <div className="mt-6">
          {activeTab === "schedule" && <ExhibitionSchedule />}

          {activeTab === "create-card" && (
            <FormExhibitionBuilder
              initialFields={location.state?.fields || []}
              seasonData={location.state?.seasonData}
              onSubmit={async (formConfig) => {
                try {
                  console.log("FORM CONFIG", formConfig);

                  // تحويل البيانات لتناسب الباك
                  const questions = formConfig.questions.map((field) => ({
                    type:
                  ...
                  // إنشاء الفورم
                  const response = await createExhibitionForm({
                    questions,
                  }).unwrap();
                  ...
                  if (!formId) {
                    throw new Error("لم يتم استلام form_id");
                  }

                  // نشر الفورم
                  await publishSeason(formId).unwrap();
                  showSuccess("تم إنشاء ونشر الفورم بنجاح");
                } catch (err) {
          ...
          {/* الكود محمي هنا وبشكل متوافق مع الفلترة البصرية العلوية */}
          {!isSecretary && activeTab === "requests" && <CardRequests />}

          {activeTab === "records" && <ExhibitionRecord />}
        </div>
      </div>
    </div>
  );
